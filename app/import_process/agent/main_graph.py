...
from app.import_process.agent.nodes.node_import_neo4j import node_import_neo4j  # 导入Neo4j：从切块提取实体关系，构建知识图谱
...
workflow.add_node("node_import_neo4j",node_import_neo4j)

# 3. 指定入口节点 workflow.add_edge(START,"node_entry")
...
workflow.add_edge("node_import_milvus", "node_import_neo4j")
workflow.add_edge("node_import_neo4j", END)
...
if __name__ == "__main__":
    ...
    test_pdf_name = os.path.join("doc", "H3C UG系列路由器 用户手册-R0130-6W102-整本手册.pdf")
    ...
    if not os.path.exists(test_pdf_path):
        ...
        test_state = ImportGraphState({
            "task_id": "test_kg_import_workflow_No1",  # 测试任务ID
